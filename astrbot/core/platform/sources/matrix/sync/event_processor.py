    ...
    async def _handle_event(self, room, event_data: dict):
        ...
        if event_type == "m.room.message":
            ...
            if self.e2ee_manager and self.e2ee_manager.is_enabled():
                ...
                # E2EE 未启用时收到加密消息属于正常情况（例如房间开启了加密），降级为调试日志避免干扰
                logger.debug(
        ...
        for event in events:
            ...
            if event_type == "m.room.encrypted":
                if self.e2ee_manager:
                    ...
                    # 在未启用 E2EE 的情况下收到加密的 to-device 消息是可能的，降级为调试日志
                    logger.debug("Received encrypted to-device message but E2EE is not enabled")
                continue
            ...
            if event_type in [
                ...
                if self.e2ee_manager:
                    ...
                    # 未启用 E2EE 时收到校验事件不需要警告
                    logger.debug(f"Received {event_type} but E2EE is not enabled")
                continue
            ...
            if event_type == "m.room_key":
                if self.e2ee_manager:
                    ...
                    logger.debug("Received m.room_key but E2EE is not enabled")
                continue
            ...
            if event_type == "m.forwarded_room_key":
                if self.e2ee_manager:
                    ...
                    logger.debug("Received m.forwarded_room_key but E2EE is not enabled")
                continue
