        ...
        for segment in chain_to_send:
            ...
            if isinstance(segment, Plain):
                ...
                try:
                    ...
                    if reply_to:
                        logger.info(
                            f"发送回复消息到房间 {room_id}，回复事件ID: {reply_to}"
                        )
                    else:
                    ...
                    # 记录发送成功
                    logger.debug(
                        f"消息发送成功，房间: {room_id}，内容预览: {segment.text[:50]}..."
                    )
                except Exception as e:
                        ...
                        if delta and delta != current_content:
                            if message_event_id:
                                ...
                                try:
                                    ...
                                    # 记录流式发送第一条消息
                                    logger.info(
                                        f"流式发送第一条消息到房间 {room_id}，事件ID: {message_event_id}"
                                    )
                                    if reply_to:
                                        logger.info(f"  回复事件ID: {reply_to}")
                                except Exception as e:
                ...
                if delta:
                    ...
                    if message_event_id and time_since_last_edit >= throttle_interval:
                        ...
                        try:
                            ...
                            # 记录流式发送消息
                            logger.info(
                                f"流式发送消息到房间 {room_id}，事件ID: {message_event_id}"
                            )
                            if reply_to:
                                logger.info(f"  回复事件ID: {reply_to}")
                        except Exception as e:
