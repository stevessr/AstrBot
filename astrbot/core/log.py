...
class LogManager:
    ...
    提供了获取默认日志记录器 logger 和设置队列处理器的方法
    """
    ...
    def GetLogger(cls, log_name: str = "default"):
        ...
        # 检查该 logger 或父级 logger 是否已经有处理器，如果已经有处理器，直接返回该 logger, 避免重复配置
        ...
        console_handler.setLevel(
            logging.DEBUG
        )  # 将日志级别设置为 DEBUG(最低级别，显示所有日志), *如果插件没有设置级别，默认为 DEBUG
